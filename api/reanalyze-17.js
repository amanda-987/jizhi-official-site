    ...
    try {
      ...
      if (result.code === 200 && result.data) {
        ...
        results.forEach((analysis, index) => {
          ...
          console.log(`   车辆信息详情: ${JSON.stringify(analysis.vehicleAnalysis || [])}`)
          console.log(`   银行卡数量: ${analysis.userBankCardAnalysis?.length || 0}`)
          console.log(`   银行卡详情: ${JSON.stringify(analysis.userBankCardAnalysis || [])}`)
          console.log(`   手机号数量: ${analysis.userPhoneAnalysis?.length || 0}`)
          console.log(`   手机号详情: ${JSON.stringify(analysis.userPhoneAnalysis || [])}`)
          console.log(`   奢侈品数量: ${analysis.restrictedConsumption?.luxury?.length || 0}`)
          console.log(`   奢侈品详情: ${JSON.stringify(analysis.restrictedConsumption?.luxury || [])}`)
