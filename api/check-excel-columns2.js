...
for (const sheetName of sheetNames) {
  if (sheetName === 'wxid_4nafo0jbiv522') {
    console.log(`\n分析工作表: ${sheetName}`)
    ...
    console.log(`包含"账号"的列:`, keys.filter(k => k.includes('账号') || k.includes('Account') || k.includes('account') || k.includes('卡号')))
    console.log(`包含"用户"的列:`, keys.filter(k => k.includes('用户')))
    console.log(`包含"对手"的列:`, keys.filter(k => k.includes('对手')))
    console.log(`包含"收/付款"的列:`, keys.filter(k => k.includes('收') || k.includes('付')))
    ...
    data.slice(0, 5).forEach((row, index) => {
      ...
      console.log('  收付相关:', Object.entries(row).filter(([k, v]) => k.includes('收') || k.includes('付')))
    })
